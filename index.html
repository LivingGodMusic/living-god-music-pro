...
button{padding:10px 14px;border:none;border-radius:8px;cursor:pointer;font-weight:500;background:#38bdf8;color:#000;transition:0.3s;}
...
function nextSong(){current=(current+1)%songs.length;play(current);} 
function prevSong(){current=(current-1+songs.length)%songs.length;play(current);} 
